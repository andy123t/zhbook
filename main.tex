...
% 进行信息设置
\title{这是一个标题 \\[10pt] \zihao{2} 这是一个很长的副标题}
\college{某某大学}
\author{某某某}   % 作者姓名
%\date{2\,0\,2\,X~年~X~月}  % 完成日期
\bioinfo{其他信息}
\pubyear{202X} % 出版年
\publisher{出版社}
...
这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字这是一大段文字.这是一大段文字这是一大段文字.
...
LaTeX 的 Table 环境一个浮动体环境, 浮动体排版与 Figure 环境类似. 作为论文, 推荐使用\index{三线表}进行排版. 一般的三线表, 标题前后有横线, 表格最后有横线. %当然也可以根据需要在合适的地方加线.
