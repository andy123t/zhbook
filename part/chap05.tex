...
LaTeX 的 table 环境是一个浮动体\index{fudongti@浮动体}, 与 figure 环境的排版方式类似. 简单的表格可以使用三线表\index{sanxianbiao@三线表}进行排版, 其结构是仅在标题的上下以及表格的最下方添加横线.

排版表格最基本的环境是 tabular 环境, 适合大多数需求. 如果想要控制表格行列间距, 可使用命令 \verb|\tabcolsep| 和 \verb|\arraystretch|, 它们分别控制表格的列间距和行间距. 例如, \verb|\setlength{\tabcolsep}{10pt}| 表示列间距为 10pt, 默认是 6pt, \verb|\renewcommand{\arraystretch}{1.2}| 表示行间距为 1.2 倍, 默认是 1 倍间距.
...
出版起止时间    &  出版工作内容 \\
\hline
20XX.XX -- 20XX.XX  &  选题策划, 明确书稿主题与出版方向 \\
\hline
20XX.XX -- 20XX.XX  &  资料整理, 制定写作提纲并完成书稿撰写 \\
\hline
20XX.XX -- 20XX.XX  &  书稿审读、修改与完善 \\
\hline
20XX.XX -- 20XX.XX  &  编辑加工、校对与排版设计 \\
\hline
20XX.XX -- 20XX.XX  &  终审定稿、印刷出版与发行推广 \\
\Xhline{2\arrayrulewidth}
\end{tabular}
\end{table}
